...
'''
Contact-Inhibition Chemotaxis cell movement
'''
def cicMovement(VEGFArray, cellArray, length, width):
    #List of cell locations
    cellList = []
    for i in range(width):
        for j in range(length):
        ...
        if j == 0:
            ...
            for k in range(1, 6):
                ...
                if i[0] - k >= 0:
                    ...
                    wallList.append(i[0] - k)
                else:
                    break

            #Cell pushes off wall
            if len(filList) == 0:
                try:
            ...
            #Cell moves if VEGF favourable
                ...
                elif abs((cNew - cOld) / cOld) < xi * math.sqrt(c0 / cOld):
                        #Binary migration variable for cell detection
                        mig = False
                        #Search for cell detected by filopodium 
                        for m in cellList:
                            #Cell found
                            if m == 1:
                                mig = True
                ...
                if ((cNew - cOld) / cOld) >= xi * math.sqrt(c0 / cOld) \
                    ...
                    for m in cellList:
                        ...
                        if m == 1:
                            ...
                            break
                    #Cell has been detected
                    if mig == True or ((width - 1) in wallList):
                        try: 
                            if cellArray[i[0] - 1, i[1] + 1] == 0 and i[0] - 1 >= 0:
                                cellArray[i[0], i[1]] = 0
                                cellArray[i[0] - 1, i[1] + 1] = 1
...
def main(): 
    ...
    for i in range (1, simTime - 1):
        ...
        if t % 12 == 0:
            cicMovement(VEGFArray, cellArray, L, W)
            ...
            ax.set_title('CIC on Stationary Domain (SS) [30h]')
            fig.colorbar(im, cax=cax)
            camera.snap()
        #Timestep
        t += 1
    anim = camera.animate()
    anim.save('CIC on Stationary Domain (SS) [30h].GIF')
...
def plotter(runs):
    ...
    pyplot.title('Average Column Occupancy for CIC (SS) on Stationary Domain [30h]')
